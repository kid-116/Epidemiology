...
class SIR:
    ...
    delta = 0.35 #Probability of a person being asymptomatic
    initial_infected = 30
    ...
    def init(self):
        ...
        for infected_person in self.asymptomatic:
            self.infected.remove(infected_person)
    ...
    def get_new_asymptomatic(self, infected_contact):
        new_asymptomatic = [
            person for person in infected_contact if random.random() <= self.delta] 
        #add them to asymptomatic compartment
        for infected_person in new_asymptomatic:
            self.asymptomatic.append(infected_person)
            self.infected.remove(infected_person)
    ...
    def get_new_recovered(self):
        ...
        new_recovered = [
            person for person in self.infected or self.asymptomatic if random.random() <= self.gamma]
    ...
    def get_new_deaths(self):
        ...
        new_deaths = [
            person for person in self.infected or self.asymptomatic if random.random() <= self.alpha]
    ...
    while days <= timespan:
        ...
        while total_count < model.df.shape[0] and model.df['timestamp'][total_count] < days * 4:
            ...
            if model.person_type(person1) == 'infected' and model.person_type(person2) == 'asymptomatic':
                ...
                if generated_video_name:
                    model.add_edge(person1, person2)
            if model.person_type(person2) == 'asymptomatic' and model.person_type(person1) == 'infected':
                infected_contact.add(person2)
                if generated_video_name:
                    model.add_edge(person1, person2)
        ...
        max_infections = max(max_infections, len(model.infected)) +  max(max_infections, len(model.asymptomatic))
